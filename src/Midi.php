...
class Midi
{
    ...
    public function getMsgCount($tn): int
    {
        return count($this->tracks[$tn]);
    }

    public function insertMsg($tn, $msgStr): void
    {
    ...
    private function _getTime($msgStr): int
    {
    ...
    public function deleteMsg($tn, $mn): void
    {
        array_splice($this->tracks[$tn], $mn, 1);
    }

    public function deleteTrack($tn): int
    {
    ...
    public function soloTrack($tn): void
    {
        if ($tn == 0) {
    ...
    public function transpose($dn): void
    {
    ...
    public function transposeTrack($tn, $dn): void
    {
        $track = $this->tracks[$tn];
        $mc = count($track);
        for ($i = 0; $i < $mc; $i++) {
    ...
    {
        $txt = trim($txt); // make unix text format
    ...
    public function importTrackTxt(string $txt, $tn): void
    {
        $txt = trim($txt); // make unix text format
        if (strpos($txt, "\r") !== false && strpos($txt, "\n") === false) { // MAC
            $txt = str_replace("\r", "\n", $txt);
    ...
    public function getScore($ttype = 0)
    {
        $str = "";
    ...
    function getTxt(int $ttype = 0): string
    {
        $timebase = $this->timebase;
        $tracks = $this->tracks;
        $tc = count($tracks);
...
//---------------------------------------------------------------
    ...
    function getXml(int $ttype = 0): string
    {
        $tracks = $this->tracks;
        $tc = count($tracks);
        $type = ($tc > 1) ? 1 : 0;
...
<TimestampType>" . ($ttype == 1 ? 'Delta' : 'Absolute') . "</TimestampType>\n";
    ...
    function importXml(string $xmlStr): void
    {
        $this->evt = array();
        $this->atr = array();
        $this->dat = '';
    ...
    function importMid(string $smf_path): void
    {
        ...
        if (substr($header, 0, 8) != "MThd\0\0\0\6") {
            _err('wrong MIDI-header');
        }
        $type = ord($header[9]);
        if ($type > 1) {
            _err('only SMF Typ 0 and 1 supported');
        }
...
// MM: ToDo: Len >127 has to be variable length-encoded !!!
    ...
    public function playMidFile(string $file, int $visible = 1, int $auto = 1, int $loop = 1, string $plug = ''): void
    {
        switch ($plug) {
            case 'qt':
                ?>
                ...
                <OBJECT CLASSID="clsid:02BF25D5-8C17-4B23-BC80-D3488ABDDC6B"
                        WIDTH="<?php echo ($visible == 0) ? 2 : 160 ?>" HEIGHT="<?php echo ($visible == 0) ? 2 : 16 ?>"
                    ...
                    <EMBED TYPE="video/quicktime" SRC="<?php echo $file ?>"
                           WIDTH="<?php echo ($visible == 0) ? 2 : 160 ?>"
                           HEIGHT="<?php echo ($visible == 0) ? 2 : 16 ?>"
                           AUTOPLAY="<?php echo $auto ? 'true' : 'false' ?>"
                           LOOP="<?php echo $loop ? 'true' : 'false' ?>"
                           CONTROLLER="<?php echo ($visible == 0) ? 'false' : 'true' ?>"
                           <?php echo ($visible == 0) ? 'HIDDEN="true" ' : '' ?>PLUGINSPAGE="http://www.apple.com/quicktime/download/">
                </OBJECT>
                <?php
                break;
            case 'wm':
                ...
                <OBJECT CLASSID="CLSID:22D6f312-B0F6-11D0-94AB-0080C74C7E95"
                    ...
                    <EMBED TYPE="video/x-ms-asf-plugin"
                           ...
                           SRC="<?php echo $file ?>" AutoStart="<?php echo $auto ? 1 : 0 ?>"
                           ShowControls="<?php echo ($visible == 0) ? 0 : 1 ?>" Loop="<?php echo $loop ?>"
                           width=<?php echo ($visible == 0) ? 0 : 300 ?> height=<?php echo ($visible == 0) ? 0 : 44 ?>>
                </OBJECT>
                <?php
                break;
            case 'bk':
                ?>
                <OBJECT CLASSID="CLSID:B384F118-18EE-11D1-95C8-00A024330339"
                        CODEBASE="http://dasdeck.de/beatnik/beatnik.cab"
                        WIDTH=<?php echo ($visible == 0) ? 2 : 144 ?> HEIGHT=<?php echo ($visible == 0) ? 0 : 15 ?>>
                    <PARAM NAME="SRC" VALUE="<?php echo $file ?>">
                    ...
                    <PARAM NAME="LOOP" VALUE="<?php echo $loop ? 'true' : 'false' ?>">
                    <?php echo ($visible == 0) ? '<PARAM NAME="HIDDEN" VALUE="true">' : '' ?>
                    <EMBED TYPE="audio/rmf" SRC="<?php echo $file ?>" WIDTH="<?php echo ($visible == 0) ? 2 : 144 ?>"
                           HEIGHT="<?php echo ($visible == 0) ? 2 : 15 ?>" DISPLAY="SONG"
                           AUTOSTART="<?php echo $auto ? 'true' : 'false' ?>"
                           LOOP="<?php echo $loop ? 'true' : 'false' ?>"
                           PLUGINSPAGE="http://www.beatnik.com/to/?player"<?php echo ($visible == 0) ? ' HIDDEN="true"' : '' ?>
                    >
                </OBJECT>
                <?php
                break;
            case 'jv':
                ...
                <OBJECT classid="clsid:CAFEEFAC-0014-0002-0000-ABCDEFFEDCBA"
                        WIDTH="<?php echo ($visible == 0) ? 0 : 32 ?>" HEIGHT="<?php echo ($visible == 0) ? 0 : 32 ?>"
                    ...
                    <COMMENT>
                        <EMBED TYPE="application/x-java-applet;jpi-version=1.4.2" CODE="MidiApplet.class"
                               WIDTH="<?php echo ($visible == 0) ? 0 : 32 ?>"
                               HEIGHT="<?php echo ($visible == 0) ? 0 : 32 ?>"
                               pluginspage="http://java.sun.com/products/plugin/index.html#download"
                               autostart="<?php echo $auto ? 1 : 0 ?>" loop="<?php echo $loop ? 1 : 0 ?>"></EMBED>
                    </COMMENT>
                </OBJECT>
                <?php
                break;
            default:
                ?>
                <EMBED SRC="<?php echo $file ?>" TYPE="audio/midi" AUTOSTART="<?php echo $auto ? 'TRUE' : 'FALSE' ?>"
                       LOOP="<?php echo $loop ? 'TRUE' : 'FALSE' ?>"<?php echo ($visible == 0) ? ' HIDDEN="true"' : '' ?>>
            <?php
        }
    }

    function downloadMidFile(string $output, $file = false): void
    {
    ...
    public function addMsg($tn, string $msgStr, $ttype = 0): void
    {
        $track = $this->tracks[$tn];
    ...
    private function _chData($parser, string $data)
    {
